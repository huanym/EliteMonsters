...
> ⚠️ **编码铁律**: 所有 `.java` `.yml` `.xml` 文件必须用 `[System.IO.File]::WriteAllBytes` 写入 UTF-8 无 BOM。
> 绝对不能用 `Set-Content -Encoding UTF8`（会加 BOM）。绝对不能用 `Get-Content` + `WriteAllText`（中文会损坏）。
...
| 配置反射同步 | `ConfigManager.load()` 用反射替换 `JavaPlugin.config` 字段, 确保 `plugin.getConfig()` 热重载生效 |
| 配置自动迁移 | 启动时检测 `config-version`, 旧版自动备份到 `backup/` 并替换新模板 |
...
└── src/main/
     ...
     └── java/com/elitemonsters/plugin/
          ...
          ├── config/
          │   ├── ConfigManager.java      # 配置加载 (反射同步/double读取修复/debugLog)
          │   └── LangManager.java
          ├── affix/
          │   ├── AffixData.java
          │   └── AffixManager.java
...
| 全局强度倍率 | `config.yml generation.global-attribute-scale` | 默认 0.5, 缩放血/攻/速 |
| 卸载恢复原状 | `EliteGenerationListener.revertAllElites()` | onDisable 时重置属性/卸甲/清名 |
...
| FLAMING_INFERNO | 范围持续燃烧 | 点燃3秒+火焰/岩浆粒子 |
| FREEZING_FROST | 范围持续减速 | 缓慢III+雪/冰粒子 |
| EXPLOSIVE_BLAST | 投掷TNT (6s CD) | 爆炸+击飞+波状粒子 |
| SHIELDED_BARRIER | HP<30%自动开盾 | 吸收50%伤害 |
| INVISIBLE_CLOAK | 15s隐身 (25s CD) | 隐身+传送门粒子 |
| REGENERATING_HEAL | HP<40%自动回血 | 持续回复+心形粒子 |
...
| GROUP | 组合奖励, 包含多个子奖励(可递归嵌套), 按顺序展开发放 |
...
- [ ] **NamespacedKey 构造函数** — `new NamespacedKey(plugin, key)` 可能在新版 API 中改为 `new NamespacedKey(namespace, key)`
...
| | **重构: 怪物名字 lang 可定制** — `name-format` + `mob-names` ╞Õ lang.yml |
| | **重构: ╩¼│▒ Title+BossBar** — ┤≤▒Ω╠Γ▓¸▒¿ + ╜°╢╚╠⌡ + ╡╣╝╞╩▒╠⌡ |
| | **重构: RGB╜Ñ▒Σ╔½** — GradientUtil + `<g:#>` ╙∩╖¿ + `&#RRGGBB` ┴∙╜°╓╞ |
| | **重构: ╩¼│▒╔·│╔░δ╛╢┐╔┼Σ╓├** — `spawn-radius` / `spawn-min-radius` |
| | **重构: ╩▄╗≈╖┤└í╧╡═│** — 11┤╩╫║╕≈╫╘╢└┴ó╩▄╗≈╨¯╣√ |
| | **API╟Î╥ã**: Sound.valueOf → Registry.SOUNDS, OldEnum.name → Keyed.getKey |
| | **BUG: ╩¼│▒╓╪╕┤┐¬╞⌠** — startHorde╖╡╗╪false+╓╕┴ε╝∞▓Γ |
| | **BUG: ╜Ñ▒Σ╔½╩º╨¯** — LegacyComponentSerializer character('&').hexColors() |
| | **BUG: ┐¬╣╪▓╗▒ú┤µ** — toggle ║≤ saveConfig() |
| | **BUG: ─ú╨═├√╜Ñ▒Σ╔½** — entity.setCustomName → entity.customName(Component) |
| | **┤╩╫║╓╨╬─╗»** — 11┤╩╫║╚ñ╬╢╓╨╬─├√ (┐±╒╜╩┐/╤╫─º/...) |
| | **BUG: ▒¼╒Î▓╗╞╞╗╡╡╪╨╬** — EntityExplodeEvent.blockList().clear() |
| | **╟°┐Θ╛½╙ó╔╧╧▐** — generation.max-per-chunk, ╩¼│▒╞┌╝Σ╚╞╣² |
| | **╨┬╓╕┴ε: /elite clear** — ╖╢╬¯/╟°┐Θ/╩└╜τ/└α╨═╫Θ║╧ |
| | **BUG: ╜▒└°╩▒╗·** — ▓Î┤╬╟σ│²║≤╖ó╖┼, ╖╟┐¬╩╝╩▒ |
| | **▓╬╒╜╜▒└°** — ╓╗╙╨┤≥/▒╗┤≥╡─═µ╝╥╗±╡├╜▒└° |
| | **╩¼│▒╔·│╔╬╗╓├╨Ì╕┤** — getSafeSpawnLocation ▒▄┐¬╦«/╤╥╜¼/╟╜▒┌ |
| | **╩¼│▒╣╓╬∩╕▀┴┴** — glow-mobs ┐╔┼Σ╓├ + HORDE_META_KEY ▒Ω╝╟ |
| | **╩¼│▒│¼╩▒╩¯░▄** — wave-timeout ┼Σ╓├ + failHorde() |
| | **center-mode ┐╔┼Σ╓├** — random_player / world_spawn |
| | **═Ω│╔╧√╧ó╧╘╩╛▓╬╒╜═µ╝╥** — %players% ▒Σ┴┐ |
...
| | **BUG: hot reload 配置不生效** — ConfigManager.load() 反射替换 JavaPlugin.config |
...
| | **PROJECT.md 全面更新** — 关键决策/架构/待办/修改记录 (本文档) |
| | 编译通过, 0 error, 1.1.0 |
...
# 输出
# target\EliteMonsters-1.1.0.jar
